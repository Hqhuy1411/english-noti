/**
 * Minimal DynamoDB access for the study table (coach's copy).
 *
 * Duplicated from the notifier rather than shared, for the CodeUri reason in
 * .claude/rules/study-data.md. This copy adds a conditional write, which the
 * notifier has no use for.
 *
 ...
 *
 * `condition` is what makes update_id de-duplication work: two retries of the
 * same Telegram update race here and exactly one wins, the loser getting a
 * ConditionalCheckFailedException that the caller reads as "already handled".
 */
export async function putItem(table, item, { condition } = {}) {
